...
System::Void c_frm_Camera_Positioning_Pose::tait_extrinsic_zyx ()
  ...
  this->ny = sin (psi) * sin (theta) * cos (phi) - cos (psi) * sin (phi);

  this->nz = cos (psi) * cos (theta) * cos (phi) + sin (psi) * sin (phi);

  this->ox = cos (theta) * sin (phi);

  this->oy = sin (psi) * sin (theta) * sin (phi) + cos (psi) * cos (phi);

  this->oz = cos (psi) * sin (theta) * sin (phi) - sin (psi) * cos (phi);
...
System::Void c_frm_Camera_Positioning_Pose::tait_intrinsic_zyx ()
  ...
  this->ox = cos (phi) * sin (theta) * sin (psi) - sin (phi) * cos (psi);

  this->oy = sin (phi) * sin (theta) * sin (psi) + cos (phi) * cos (psi);

  this->oz = cos (theta) * sin (psi);

  this->ax = cos (phi) * sin (theta) * cos (psi) + sin (phi) * sin (psi);

  this->ay = sin (phi) * sin (theta) * cos (psi) - cos (phi) * sin (psi);
...
System::Void c_frm_Camera_Positioning_Pose::euler_extrinsic_zyz ()
  ...
  this->nx = cos (psi) * cos (theta) * cos (phi) - sin (psi) * sin (phi);

  this->ny = sin (psi) * cos (theta) * cos (phi) + cos (psi) * sin (phi);

  this->nz = -sin (theta) * cos (phi);

  this->ox = -cos (psi) * cos (theta) * sin (phi) - sin (psi) * cos (phi);

  this->oy = -sin (psi) * cos (theta) * sin (phi) + cos (psi) * cos (phi);
...
System::Void c_frm_Camera_Positioning_Pose::euler_intrinsic_zyz ()
  ...
  this->nx = cos (phi) * cos (theta) * cos (psi) - sin (phi) * sin (psi);

  this->ny = sin (phi) * cos (theta) * cos (psi) + cos (phi) * sin (psi);

  this->nz = -sin (theta) * cos (psi);

  this->ox = -cos (phi) * cos (theta) * sin (psi) - sin (phi) * cos (psi);

  this->oy = -sin (phi) * cos (theta) * sin (psi) + cos (phi) * cos (psi);
